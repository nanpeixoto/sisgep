...
public class RelatorioSiredUtil   {
	...
	public void preExportar() {
		...
		for (int i = 1; i <= sheet.getLastRowNum(); i++) {
			...
			for (int j = 0; j < row.getPhysicalNumberOfCells(); j++) {
				...
				cell.setCellStyle(getEstiloLinhasTabela(wb));
			}
		}
	...
	private HSSFCellStyle getEstiloLinhasTabela(HSSFWorkbook wb) {
		...
		linhaEstiloBrancoSemNegrito.setFillForegroundColor(HSSFColor.GREY_25_PERCENT.index);
	...
	private void criarCabecalhoPadraoRelatorio(HSSFSheet sheet, HSSFWorkbook wb, int quantidadeColunas) {
		...
		cell1.setCellValue(new HSSFRichTextString(MensagemUtil.obterMensagem("geral.nomeSistemaCompleto")));
